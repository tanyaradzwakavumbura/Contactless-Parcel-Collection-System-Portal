...
if(!isset($_SESSION["daily_hourly_report"])){
    header("location:hourly_statistics_per_date_generator_search");
}else{
    $the_date = $_SESSION["daily_hourly_report"];
}
?>
<!DOCTYPE html>
<html lang="en">
                    ...
                    <div>
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb breadcrumb-style1 mg-b-10">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">Daily Hourly Report for <?php echo format_date($the_date);?></li>
                            </ol>
                        </nav>
                        <h4 class="mg-b-0 tx-spacing--1">Daily Hourly Report for <?php echo format_date($the_date);?></h4>
                    </div>
                    ...
                    <div data-label="Example" class="df-example demo-table col-md-12">
                        ...
                        <table id="example1" class="table table-hover table-bordered">
                            ...
                            <tbody>
                                ...
                                <?php
                                $find_all_times = $connect->prepare("SELECT DISTINCT HOUR(time) AS distinct_hours FROM vehicle_entry WHERE date=?");
                                $find_all_times->execute([$the_date]);
                                while($row=$find_all_times->fetch(PDO::FETCH_ASSOC)){
                                    
                                    $distinct_hours = $row["distinct_hours"];
                                    $distinct_hours = $distinct_hours.":00 Hrs";
                                    
                                    $the_distinct_hours = $row["distinct_hours"];

                                    $closed = "closed";

                                  
                                    $find_all_vehicles = $connect->prepare("
    SELECT DISTINCT code, transaction, amount 
    FROM vehicle_entry 
    WHERE date = ? 
    AND HOUR(time) = ?
");
$find_all_vehicles->execute([$the_date, $the_distinct_hours]);
                                    ...
                                    while($row=$find_all_vehicles->fetch(PDO::FETCH_ASSOC)){
                                        ...
                                        $the_found_transaction_amount = $row["amount"];

                                        $total_costs = $total_costs + $the_found_transaction_amount;
                                    }
                                    
                                    $pre_desc = "Prepayment";
                                    //  //SELECT * FROM vehicles WHERE status=? AND date_now=? AND HOUR(date_time)=?
                                    $find_prepayments_query = $connect->prepare("SELECT SUM(amount) AS total_month_amount_prepayment FROM transactions WHERE description=? AND date=? AND HOUR(date)=?");
                                    $find_prepayments_query->execute([$pre_desc,$the_date,$the_distinct_hours]);
                                ...
                                <tr>
                                    ...
                                    while($row=$find_vehicle_categories->fetch(PDO::FETCH_ASSOC)){
                                        ...
                                        $stat = "closed";
                                        $find_vehicle_details = $connect->prepare("
    SELECT DISTINCT code, transaction 
    FROM vehicle_entry 
    WHERE date = ? 
    AND HOUR(date) = ? 
    AND vehicle IN (
...
");
$find_vehicle_details->execute([$the_date, $the_distinct_hours, $ve_cat_code]);
