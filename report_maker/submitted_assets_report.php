...
if(isset($_POST["generate_submitted_assests_disposal_report"])){
...
}elseif(isset($_POST["submit_submitted_to_disposal_history"])){
     $update_disposed_assets = $connect->prepare("UPDATE disposal_requests SET status=? WHERE status=?");
     $current_stat = "submitted";
     $new_stat = "disposed";
     $update_disposed_assets->execute([$new_stat,$current_stat]);  
    
     echo "<script type='text/javascript'>window.location.href='../approved_assets_disposal'</script>";
}
