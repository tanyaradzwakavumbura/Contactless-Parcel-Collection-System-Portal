...
$s = "first_timer";
$find_all_full_members = $connect->prepare("SELECT * FROM members WHERE member_status=?");
$find_all_full_members->execute([$s]);
$count_all_full_members = $find_all_full_members->rowCount();
?>
...
<body>
                    ...
                    <div>
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb breadcrumb-style1 mg-b-10">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">Full Members Report</li>
                            </ol>
                        </nav>
                        <h4 class="mg-b-0 tx-spacing--1">Full Members Report</h4>
                    ...
                    <div class="row row-xs">
                        <div class="container mt-4">
                            <header class="text-center mb-4">
                                <h2 class="display-6">First Timers Members Report</h2>
                                <p class="lead mb-0">Total Members: <?php echo $count_all_full_members;?></p>
                            </header>
                                                ...
                                                <table class="table table-striped mg-b-0 table-bordered table-hover">
                                                    <thead class="thead-primary">
                                                        <tr>
                                                            ...
                                                            <th scope="col">Kingschat</th>
                                                          
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                      <?php
                                                      //SELECT `id`, `code`, `fullname`, `phone_number`, `email`, `address`, `kingschat_handle`, `foundation_school`, `baptism_status`, `cell_group`, `service_department`, `member_status`, `date`, `gender`, `dob` FROM `members` WHERE 1
                                                      $find_all_full_members = $connect->prepare("SELECT * FROM members WHERE member_status=?");
                                                      $find_all_full_members->execute([$s]);
                                                      while($row=$find_all_full_members->fetch(PDO::FETCH_ASSOC)){
                                                        $member_fullname = $row["fullname"];
                                                        $member_dob = $row["dob"];
                                                        $member_phone = $row["phone_number"];
                                                        $member_email = $row["email"];
                                                        $member_address = $row["address"];
                                                        $member_kingschat = $row["kingschat_handle"];
                                                        ?>
                                                        <tr>
                                                            ...
                                                            <td><?php echo $member_kingschat;?></td>
                                                        </tr>
                                                        <?php
                                                      }
                                                      ?>
    ...
    function generateReport() {
        ...
        const opt = {
            ...
            filename: "FIrst TImers Report.pdf",
            image: {
                type: 'jpeg',
                quality: 0.5
