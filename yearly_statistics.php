                    ...
                    <div>
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb breadcrumb-style1 mg-b-10">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">Yearly Statistics</li>
                            </ol>
                        </nav>
                        <h4 class="mg-b-0 tx-spacing--1">Yearly Statistics</h4>
                    </div>
                    ...
                    <div data-label="Example" class="df-example demo-table col-md-12">
                        ...
                        <table id="example1" class="table table-hover table-bordered">
                            ...
                            <tbody>
                                ...
                                <?php
                                $find_months = $connect->prepare("SELECT DISTINCT YEAR(date) as the_year FROM vehicle_entry  ORDER BY the_year DESC");
                                $find_months->execute();
                                while($row=$find_months->fetch(PDO::FETCH_ASSOC)){
                                    ...
                                    $closed = "closed";
                                    $find_all_vehicles = $connect->prepare("SELECT DISTINCT code, transaction FROM vehicle_entry WHERE YEAR(date)=?");
                                    $find_all_vehicles->execute([$the_year]);
                                    ...
                                    $pre_desc = "Prepayment";
                                    $find_prepayments_query = $connect->prepare("SELECT SUM(amount) AS total_month_amount_prepayment FROM transactions WHERE description=? AND YEAR(date)=?");
                                    $find_prepayments_query->execute([$pre_desc,$the_year]);
                                ...
                                <tr>
                                    ...
                                    while($row=$find_vehicle_categories->fetch(PDO::FETCH_ASSOC)){
                                        ...
                                        $stat = "closed";
                                        $find_vehicle_details = $connect->prepare("SELECT DISTINCT code, transaction FROM vehicle_entry WHERE YEAR(date)=? AND vehicle IN (SELECT plate FROM vehicles WHERE category=?)");
                                        $find_vehicle_details->execute([$the_year,$ve_cat_code]);
