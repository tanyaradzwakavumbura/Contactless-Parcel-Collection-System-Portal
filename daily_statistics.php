...
if(!isset($_SESSION["daily_start_date"])){
    header("location:daily_statistics_generator_search");
}else{
    $start_date = $_SESSION["daily_start_date"];
    $end_date = $_SESSION["daily_end_date"];
}
?>
<!DOCTYPE html>
<html lang="en">
                    ...
                    <div>
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb breadcrumb-style1 mg-b-10">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">Daily Statistics for <?php echo format_date($start_date);?> to <?php echo format_date($end_date);?></li>
                            </ol>
                        </nav>
                        <h4 class="mg-b-0 tx-spacing--1">Daily Statistics for <?php echo format_date($start_date);?> to <?php echo format_date($end_date);?></h4>
                    </div>
                    ...
                    <div data-label="Example" class="df-example demo-table col-md-12">
                        ...
                        <table id="example1" class="table table-hover table-bordered">
                            ...
                            <tbody>
                                ...
                                <?php
                                $find_months = $connect->prepare("SELECT DISTINCT date FROM vehicle_entry WHERE date BETWEEN ? AND ? ORDER BY date ASC");
                                $find_months->execute([$start_date,$end_date]);
                                while($row=$find_months->fetch(PDO::FETCH_ASSOC)){
                                    $the_date = $row["date"];

                                    $closed = "closed";
                                    $find_all_vehicles = $connect->prepare("SELECT DISTINCT code, transaction FROM vehicle_entry WHERE date=?");
                                    $find_all_vehicles->execute([$the_date]);
                                    ...
                                    $pre_desc = "Prepayment";
                                    $find_prepayments_query = $connect->prepare("SELECT SUM(amount) AS total_month_amount_prepayment FROM transactions WHERE description=? AND date=?");
                                    $find_prepayments_query->execute([$pre_desc,$the_date]);
                                ...
                                <tr>
                                    ...
                                    while($row=$find_vehicle_categories->fetch(PDO::FETCH_ASSOC)){
                                        ...
                                        $stat = "closed";
                                        $find_vehicle_details = $connect->prepare("SELECT DISTINCT code, transaction FROM vehicle_entry WHERE date=? AND vehicle IN (SELECT plate FROM vehicles WHERE category=?)");
                                        $find_vehicle_details->execute([$the_date,$ve_cat_code]);
